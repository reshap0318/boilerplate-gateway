...
export const useRoleStore = defineStore('role', () => {
  ...
  async function fetchAllRoles(): Promise<IRole[]> {
    try {
      const { data } = await get<IApiResponse<IRole[]>>('/api/svc/uam/roles')
      return data.data || []
    } catch (error: any) {
  ...
  async function fetchRolePermissions(id: number): Promise<IPermission[]> {
    try {
      const { data } = await get<IApiResponse<IPermission[]>>(
        `/api/svc/uam/roles/${id}/permissions`,
      )
      return data.data || []
    } catch (error: any) {
