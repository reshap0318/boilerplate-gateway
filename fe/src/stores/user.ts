...
export const useUserStore = defineStore('user', () => {
  ...
  async function fetchAllUsers(): Promise<IUser[]> {
    try {
      const { data } = await get<IApiResponse<IUser[]>>('/api/svc/uam/users')
      return data.data || []
    } catch (error: any) {
  ...
  async function updateStatus(id: number, status: TUserStatus): Promise<IUser | null> {
    try {
      const { data } = await put<IApiResponse<IUser>>(`/api/svc/uam/users/${id}/status`, { status })
      return data.data
    } catch (error: any) {
