...
// LogCtx logs a message tagged with the request's trace id (see
// middleware.TraceID), so every log line for one request can be grepped
// across this service — and across other services, if they log the same id.
// Usage: logger.LogCtx(ctx, "CategoryCreate", "Created category: %s", name)
func (l *Logger) LogCtx(ctx context.Context, function, format string, v ...interface{}) {
	traceID := GetTraceID(ctx)
	if traceID == "" {
		l.Printf("[%s] %s", function, fmt.Sprintf(format, v...))
		return
	}
	l.Printf("[trace:%s] [%s] %s", traceID, function, fmt.Sprintf(format, v...))
}
