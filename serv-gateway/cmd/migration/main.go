...
import (
	...
	"github.com/reshap0318/serv-gateway/cmd/migration/seeders"
...
func runSeed(db *gorm.DB) {
	...
	// User/Role/Permission seed data now lives in serv-uam's own seeder — see
	// serv-uam/cmd/migration/seeders. Only gateway's own config (GatewayService/
	// GatewayRoute) is seeded here.
	seeders.SeedUamRoutes(db, seeders.SeedUamService(db))
	seeders.SeedMessageRoutes(db, seeders.SeedMessageService(db))
